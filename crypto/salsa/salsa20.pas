...
(*-------------------------------------------------------------------------
 ...
 Portions of ChaCha code (C) Copyright 2010 EddyHawk
...
procedure salsa20_wordtobyte(var output: T64B; {$ifdef CONST} const {$else} var {$endif} input: TSalsaBlk;
  ...
  {-This is the Salsa20 "hash" function, add}
var
  i: integer;
  x: TSalsaBlk;
begin
...
begin
  x := input;
  for i:=1 to (rounds shr 1) do begin
    x[ 0] := x[ 0]+x[ 4];   x[12] := x[12] xor x[ 0];   x[12] := (x[12] shl 16) or (x[12] shr (32-16));
    x[ 8] := x[ 8]+x[12];   x[ 4] := x[ 4] xor x[ 8];   x[ 4] := (x[ 4] shl 12) or (x[ 4] shr (32-12));
    x[ 0] := x[ 0]+x[ 4];   x[12] := x[12] xor x[ 0];   x[12] := (x[12] shl  8) or (x[12] shr (32- 8));
    x[ 8] := x[ 8]+x[12];   x[ 4] := x[ 4] xor x[ 8];   x[ 4] := (x[ 4] shl  7) or (x[ 4] shr (32- 7));

    x[ 1] := x[ 1]+x[ 5];   x[13] := x[13] xor x[ 1];   x[13] := (x[13] shl 16) or (x[13] shr (32-16));
    x[ 9] := x[ 9]+x[13];   x[ 5] := x[ 5] xor x[ 9];   x[ 5] := (x[ 5] shl 12) or (x[ 5] shr (32-12));
    x[ 1] := x[ 1]+x[ 5];   x[13] := x[13] xor x[ 1];   x[13] := (x[13] shl  8) or (x[13] shr (32- 8));
    x[ 9] := x[ 9]+x[13];   x[ 5] := x[ 5] xor x[ 9];   x[ 5] := (x[ 5] shl  7) or (x[ 5] shr (32- 7));

    x[ 2] := x[ 2]+x[ 6];   x[14] := x[14] xor x[ 2];   x[14] := (x[14] shl 16) or (x[14] shr (32-16));
    x[10] := x[10]+x[14];   x[ 6] := x[ 6] xor x[10];   x[ 6] := (x[ 6] shl 12) or (x[ 6] shr (32-12));
    x[ 2] := x[ 2]+x[ 6];   x[14] := x[14] xor x[ 2];   x[14] := (x[14] shl  8) or (x[14] shr (32- 8));
    x[10] := x[10]+x[14];   x[ 6] := x[ 6] xor x[10];   x[ 6] := (x[ 6] shl  7) or (x[ 6] shr (32- 7));

    x[ 3] := x[ 3]+x[ 7];   x[15] := x[15] xor x[ 3];   x[15] := (x[15] shl 16) or (x[15] shr (32-16));
    x[11] := x[11]+x[15];   x[ 7] := x[ 7] xor x[11];   x[ 7] := (x[ 7] shl 12) or (x[ 7] shr (32-12));
    x[ 3] := x[ 3]+x[ 7];   x[15] := x[15] xor x[ 3];   x[15] := (x[15] shl  8) or (x[15] shr (32- 8));
    x[11] := x[11]+x[15];   x[ 7] := x[ 7] xor x[11];   x[ 7] := (x[ 7] shl  7) or (x[ 7] shr (32- 7));

    x[ 0] := x[ 0]+x[ 5];   x[15] := x[15] xor x[ 0];   x[15] := (x[15] shl 16) or (x[15] shr (32-16));
    x[10] := x[10]+x[15];   x[ 5] := x[ 5] xor x[10];   x[ 5] := (x[ 5] shl 12) or (x[ 5] shr (32-12));
    x[ 0] := x[ 0]+x[ 5];   x[15] := x[15] xor x[ 0];   x[15] := (x[15] shl  8) or (x[15] shr (32- 8));
    x[10] := x[10]+x[15];   x[ 5] := x[ 5] xor x[10];   x[ 5] := (x[ 5] shl  7) or (x[ 5] shr (32- 7));

    x[ 1] := x[ 1]+x[ 6];   x[12] := x[12] xor x[ 1];   x[12] := (x[12] shl 16) or (x[12] shr (32-16));
    x[11] := x[11]+x[12];   x[ 6] := x[ 6] xor x[11];   x[ 6] := (x[ 6] shl 12) or (x[ 6] shr (32-12));
    x[ 1] := x[ 1]+x[ 6];   x[12] := x[12] xor x[ 1];   x[12] := (x[12] shl  8) or (x[12] shr (32- 8));
    x[11] := x[11]+x[12];   x[ 6] := x[ 6] xor x[11];   x[ 6] := (x[ 6] shl  7) or (x[ 6] shr (32- 7));

    x[ 2] := x[ 2]+x[ 7];   x[13] := x[13] xor x[ 2];   x[13] := (x[13] shl 16) or (x[13] shr (32-16));
    x[ 8] := x[ 8]+x[13];   x[ 7] := x[ 7] xor x[ 8];   x[ 7] := (x[ 7] shl 12) or (x[ 7] shr (32-12));
    x[ 2] := x[ 2]+x[ 7];   x[13] := x[13] xor x[ 2];   x[13] := (x[13] shl  8) or (x[13] shr (32- 8));
    x[ 8] := x[ 8]+x[13];   x[ 7] := x[ 7] xor x[ 8];   x[ 7] := (x[ 7] shl  7) or (x[ 7] shr (32- 7));

    x[ 3] := x[ 3]+x[ 4];   x[14] := x[14] xor x[ 3];   x[14] := (x[14] shl 16) or (x[14] shr (32-16));
    x[ 9] := x[ 9]+x[14];   x[ 4] := x[ 4] xor x[ 9];   x[ 4] := (x[ 4] shl 12) or (x[ 4] shr (32-12));
    x[ 3] := x[ 3]+x[ 4];   x[14] := x[14] xor x[ 3];   x[14] := (x[14] shl  8) or (x[14] shr (32- 8));
    x[ 9] := x[ 9]+x[14];   x[ 4] := x[ 4] xor x[ 9];   x[ 4] := (x[ 4] shl  7) or (x[ 4] shr (32- 7));
  end;
