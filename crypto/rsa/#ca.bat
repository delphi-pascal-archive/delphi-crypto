...
set PCB=call fpc240 -B -dMPC_MAXRadix64
del %SRC%.exe >nul
%PCB% %OPT% %SRC%.pas
if not (%1%)==() pause
echo. >>%LOG%
...
set PCB=call fpc242d -B -dMPC_MAXRadix64
del %SRC%.exe >nul
%PCB% %OPT% %SRC%.pas
if not (%1%)==() pause
echo. >>%LOG%
...
set PCB=D:\DMX\M7\DCC32.EXE -b  -dMPC_MAXRadix64
del %SRC%.exe >nul
%PCB% %OPT% %SRC%.pas
if not (%1%)==() pause
echo. >>%LOG%
...
set PCB=D:\DMX\M12\DCC32.EXE -b   -dMPC_MAXRadix64
del %SRC%.exe >nul
%PCB% %SRC%.pas
if not (%1%)==() pause
echo. >>%LOG%
