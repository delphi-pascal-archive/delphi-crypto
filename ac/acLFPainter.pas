...
type
  ...
  TcxACLookAndFeelPainter = class(TcxStandardLookAndFeelPainter) // TcxFlatLookAndFeelPainter)
  public
    // colors
...
procedure Debugalert(const s : string); begin {$IFDEF ACDEBUG} Alert(s); {$ENDIF} end;
const
...
begin // -
  if Skinned then begin 
    i := DefaultManager.GetMaskIndex(DefaultManager.ConstData.IndexGLobalInfo, s_GLobalInfo, s_SmallBoxChecked);
    if i < 0 then Result := inherited CheckButtonSize else begin
...
begin // +
  ...
  else Result := DefaultEditorTextColor(AIsDisabled);
end;
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      ...
      DrawText(TmpBmp.Canvas.Handle, PChar(ACaption), Length(ACaption), rText, DT_EXPANDTABS + DT_VCENTER + DT_CENTER + DT_SINGLELINE);
...
begin
  if Skinned then begin
    case ACheckState of
      cbsUnchecked : i := DefaultManager.GetMaskIndex(DefaultManager.ConstData.IndexGLobalInfo, s_GLobalInfo, s_SmallBoxUnChecked);
    ...
    if DefaultManager.IsValidImgIndex(i) then begin
      ...
      w := WidthOfImage(DefaultManager.ma[i]);
      h := HeightOfImage(DefaultManager.ma[i]);
      ...
      DrawSkinGlyph(TmpBmp, Point(NewLeft, NewTop), State, 1, DefaultManager.ma[i], CI);
...
begin
  inherited;
  Debugalert('DrawGroupBoxBackground');
end;
...
begin
  inherited;
  Debugalert('DrawGroupBoxCaption');
end;

class procedure TcxACLookAndFeelPainter.DrawGroupBoxContent(ACanvas: TcxCanvas; ABorderRect: TRect; ACaptionPosition: TcxGroupBoxCaptionPosition);
begin
  inherited;
  Debugalert('DrawGroupBoxContent');
end;}
...
class procedure TcxACLookAndFeelPainter.DrawHeaderControlSectionContent(
  ACanvas: TcxCanvas; const ABounds, ATextAreaBounds: TRect;
...
{class procedure TcxACLookAndFeelPainter.DrawSplitter(ACanvas: TcxCanvas; const ARect: TRect; AHighlighted, AClicked, AHorizontal: Boolean);
begin
  inherited;
end;}
...
{  if Skinned then begin
    ...
    else Result := inherited FilterDropDownButtonSize;
  end
  else}
...
class function TcxACLookAndFeelPainter.GroupBoxTextColor(ACaptionPosition: TcxGroupBoxCaptionPosition): TColor;
begin
  if Skinned then Result := DefaultManager.GetGlobalFontColor else Result := inherited GroupBoxTextColor(ACaptionPosition)
end;
...
class procedure TcxACLookAndFeelPainter.SchedulerNavigationButtonSizes(AIsNextButton: Boolean; var ABorders: TRect; var AArrowSize: TSize);
begin // +
  inherited;
end;
