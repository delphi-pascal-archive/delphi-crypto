...
begin
  ...
  if GetEnumValue(TypeInfo(TacRootFolder), NewRoot) >= 0 then begin
    ...
    Flags := 0;
    P := StringToOleStr(ExpandFilename(NewRoot)); // v5.05
    HR := DesktopShellFolder.ParseDisplayName(0, nil, P, NumChars, NewPIDL, Flags);
  end;

  if HR <> S_OK then begin
    ErrorMsg := Format('Error Setting Path', [ NewRoot ] );
    NewRoot := OldRoot;
...
begin
  ...
  while not Terminated do begin
    ...
    Handles[1] := FMutex;
    Obj := WaitForMultipleObjects(2, @Handles, False, INFINITE); // v5.04 // 1000);//
    case Obj of
      WAIT_OBJECT_0 : begin
...
begin
  ...
  Flags := 0;
  P := StringToOleStr(ExpandFilename(Value)); // v5.05
  try
...
{$ENDIF}
      if Assigned(Node) then begin
        if Assigned(FListView) then FListView.TreeUpdate(TacShellFolder(Node.Data).AbsoluteID);
        if Assigned(FComboBox) then FComboBox.TreeUpdate(TacShellFolder(Node.Data).AbsoluteID);
      end;
    finally
...
begin
  case Message.Msg of
    CM_SHOWINGCHANGED : begin
      ImageList_SetBkColor(FImage, ColorToRGB(Color));
      inherited;
    end;
    WM_INITMENUPOPUP,
    WM_MENUCHAR,
  ...
  if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
    AC_REFRESH : ImageList_SetBkColor(FImage, Color);
  end; // v5.05
end;
...
begin
  ...
  if not Assigned(Images)
    then if FUseShellImages then ImageListHandle := FImage else ImageListHandle := 0
...
begin
  if FRootFolder <> nil then FreeAndNil(FRootFolder); // v4.54
  ClearItems;
  inherited;
end;
...
begin
  ...
  FRootFolder := nil;
  FImagesHandle := SHGetFileInfo('C:\', 0, FileInfo, SizeOf(FileInfo), SHGFI_SYSICONINDEX or SHGFI_SMALLICON);
  ImageList_GetIconSize(FImagesHandle, FImageWidth, FImageHeight);
//  ImageList_SetBkColor(FImagesHandle, clNone);
  FUpdating := False;
//  Style := csDropDownList;//csExDropDown;
  FObjectTypes := [otFolders];
  FRoot := SRFDesktop;
  FUseShellImages := True;
end;
...
begin
  inherited CreateWnd;
  if FImagesHandle <> 0 then SendMessage(Handle, CBEM_SETIMAGELIST, 0, FImagesHandle);
...
begin
  ...
  end else if FTreeView <> nil then FTreeView.FComboBox := nil;
...
begin
  ...
  if Value <> nil then begin
    Value.Root := Root;
    Value.FComboBox := Self;
  end else if FListView <> nil then FListView.FComboBox := nil;
...
begin
  ...
  if (Operation = opRemove) then begin
    if (AComponent = FTreeView) then FTreeView := nil
      else if (AComponent = FListView) then FListView := nil
...
begin
  ...
  if (Result = 0) and (TacShellFolder(Item1).ParenTacShellFolder <> nil) then
    Result := Smallint(
                  TacShellFolder(Item1).ParenTacShellFolder.CompareIDs(
                  0,
                  TacShellFolder(Item1).RelativeID,
                  TacShellFolder(Item2).RelativeID)
              );
end;
...
begin
  ...
  try
    List := TList.Create;
    while EnumList.Next(1, ID, LongWord(NumIDs)) = S_OK do List.Add(ID);
...
//!!  Result := Items.IndexOf(ItemsEx[Result].Caption)
end;
...
begin
  ...
  Flags := 0;
  P := StringToOleStr(ExpandFilename(s)); // v5.05
  try
    OLECheck(DesktopShellFolder.ParseDisplayName(0, nil, P, NumChars, NewPIDL, Flags));
...
begin
  ...
  if FUseShellImages and not Assigned(FImageList) then ImageList_GetIconSize(FImagesHandle, FImageWidth, FImageHeight)
  else if not Assigned(FImageList) then begin
      FImageWidth := 16;
      FImageHeight := 16;
    end
    else begin
...
begin
  ...
  if UseShellImages and (SmallImages <> nil) then Images := nil;
  inherited Destroy;
...
//  if irText in Request then v5.02
...
finalization
  ...
  OleUninitialize;
  if Assigned(SmallImages) then FreeAndNil(SmallImages);
